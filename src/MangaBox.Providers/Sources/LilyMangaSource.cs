...
public interface ILilyMangaSource : IMangaSource { }

internal class LilyMangaSource(
	IApiService _api,
	ILogger<LilyMangaSource> _logger) : BaseMangaSource<LilyMangaSource>, ILilyMangaSource
{
	public override string HomeUrl => "https://lilymanga.net/";

	public string MangaBaseUri => $"{HomeUrl}gl/";

	public override string Provider => "lily-manga";

	public override string Name => "Lily Manga (lilymanga.net)";
	...
	public override async Task<ImportManga?> Manga(string id, CancellationToken token)
	{
		...
		{
			_logger.LogWarning("Could not find title for Lily Manga id: {MangaId}", id);
			return null;
		}

		var details = ParseDetails(doc);
		var tags = details.GetValuesOrDefault("genre")
			.Concat(details.GetValuesOrDefault("tag"))
		...
		var manga = new ImportManga
		{
			...
			Tags = tags,
			Rating = tags.Any(x => x.Equals("18+", StringComparison.OrdinalIgnoreCase))
				? ContentRating.Pornographic
				: ContentRating.Safe,
			Nsfw = tags.Any(x => x.Equals("18+", StringComparison.OrdinalIgnoreCase)),
			Referer = Referer,
			Attributes = BuildAttributes(details)
		};

		var chapters = await GetChapters(url, token);
	...
	public override (bool matches, string? part) MatchesProvider(string url)
	{
		...
		if (parts.Length < 2 || !parts[0].Equals("gl", StringComparison.InvariantCultureIgnoreCase))
			return (false, null);
	...
	private async Task<HtmlDocument?> GetHtml(string url, CancellationToken token)
	{
		...
		{
			_logger.LogWarning(ex, "Failed to retrieve Lily Manga page: {Url}", url);
			return null;
		}
	}

	private async Task<List<ImportChapter>> GetChapters(string mangaUrl, CancellationToken token)
	{
		try
		{
			var chaptersUrl = $"{mangaUrl.TrimEnd('/')}/ajax/chapters/";
			var doc = await _api.PostHtml(chaptersUrl, HeadersFor(mangaUrl), token);
			return ParseChapters(doc);
		}
		catch (Exception ex)
		{
			_logger.LogWarning(ex, "Failed to retrieve Lily Manga chapters: {MangaUrl}", mangaUrl);
			return [];
		}
	}
	...
	private Action<HttpRequestMessage> HeadersFor(string referer) => request =>
	{
		...
		request.Headers.TryAddWithoutValidation("Origin", HomeUrl.TrimEnd('/'));
	};
	...
	{
		return [..details
			.Where(x => x.Key is "type" or "status" or "release" or "rank" or "rating")
	...
	private static double? ExtractChapterNumber(string? value)
	{
		...
		var match = Regex.Match(value, @"\b(?:chapter|ch|episode|ep)\s*(?<num>\d+(?:\.\d+)?)\b", RegexOptions.IgnoreCase);
		if (!match.Success)
	...
	private static string? MangaIdFromUrl(string url)
	{
		...
		var gl = Array.FindIndex(parts, x => x.Equals("gl", StringComparison.OrdinalIgnoreCase));
		return gl >= 0 && parts.Length > gl + 1 ? parts[gl + 1] : null;
	}
	...
	private static string? CleanTitle(string? value)
	{
		value = Clean(value);
		return value?
			.Replace("&#8902; Lily Manga", "", StringComparison.InvariantCultureIgnoreCase)
			.Replace("Lily Manga", "", StringComparison.InvariantCultureIgnoreCase)
			.Trim('\u22c6', '*', '-', '|')
			.Trim();
	}
...
internal static class LilyMangaDictionaryExtensions
{
	public static string? GetText(this Dictionary<string, List<string>> values, string key)
	{
		return values.TryGetValue(key, out var items)
			? items.FirstOrDefault()
			: null;
	}

	public static IEnumerable<string> GetValuesOrDefault(this Dictionary<string, List<string>> values, string key)
	{
		return values.TryGetValue(key, out var items)
			? items
			: [];

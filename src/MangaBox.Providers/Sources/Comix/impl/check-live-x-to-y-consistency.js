...
const states = [s0, ...oracle.slice(0, 5)];
...
for (let step = 0; step < 5; step++) {
  ...
  for (const [role, rc4Key, xorKey] of [['A', tri.a, tri.b], ['B', tri.b, tri.a]]) {
	...
	const mapsByPos = Array.from({ length: 10 }, () => new Map()); // key x -> y

	for (let i = 0; i < I.length; i++) {
	  const outIdx = i < tri.pc ? (2 * i + 1) : (tri.pc + i);
	...
	const coverages = mapsByPos.map(m => m.size);
	console.log(`step ${step + 1} round ${logicalRound} role ${role} conflicts=${conflicts} mappings=${mappings} coverage=${coverages.join(',')}`);
  }
}

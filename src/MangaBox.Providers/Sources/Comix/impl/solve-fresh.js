// Solve for fresh masks using only new-session oracle tokens
'use strict';

// All tokens captured in the latest browser session
const oracles = [
  ...
  // from earlier same session (collect-more-5char-tokens first batch)
...
function solvGF2(A, b_col) {
  ...
  for (let r = row; r < m; r++) if (mat[r][n] === 1) return null; // inconsistent
  const x = new Array(n).fill(0);
  for (const { row: r, col: c } of pivotCol) x[c] = mat[r][n];
  return x;
}

// Confirm all tokens share the same prefix and suffix
...
for (const b of allBytes) {
  for (let i = 0; i < 49; i++) if (b[i] !== prefix49[i]) { pfxOk = false; break; }
  for (let i = 0; i < 11; i++) if (b[54+i] !== suffix11[i]) { sfxOk = false; break; }
}
console.log('Prefix consistent:', pfxOk);
console.log('Suffix consistent:', sfxOk);
if (pfxOk) console.log('Prefix bytes:', JSON.stringify(prefix49));
if (sfxOk) console.log('Suffix bytes:', JSON.stringify(suffix11));

// Build GF2 system
const A = oracles.map(([id]) => buildFB(id, 5));
const tokens = allBytes;
...
for (let ob = 0; ob < 40; ob++) {
  ...
  if (!sol) {
	console.log('INCONSISTENT at output bit', ob, '- tokens from mixed sessions?');
	inconsistentCount++;
	masks.push(0n);
  } else {
	masks.push(maskBitsToNum(sol));
  }
}
console.log('Inconsistent bits:', inconsistentCount);
...
console.log(`\nVerification: ${pass}/${oracles.length} pass, ${fail} fail`);

if (pass === oracles.length) {
  console.log('\n=== C# LiveVariableBitMasks (40 elements for 5-char IDs) ===');
  for (let i = 0; i < 40; i += 8)
	console.log('\t\t' + masks.slice(i, i+8).map(n => n.toString()).join(', ') + ',');

  console.log('\n=== Prefix bytes (verify unchanged) ===');
  console.log(JSON.stringify(prefix49));
  console.log('\n=== Suffix bytes ===');
  console.log(JSON.stringify(suffix11));
}

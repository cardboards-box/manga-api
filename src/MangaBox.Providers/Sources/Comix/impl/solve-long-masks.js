// Solve for correct long bitmasks using 52 oracle tokens
const oracles_long = [
...
function buildFeatureBits(mangaId, charsToUse) {
  ...
  for (let i = 0; i < charsToUse; i++) {
	...
	for (let bit = 0; bit < 8; bit++) bits.push((ch >> bit) & 1);
  }
  return bits;
}
...
const LONG_BITS = 40;
const A = oracles_long.map(([id]) => buildFeatureBits(id, 5));
const tokens = oracles_long.map(([, tok]) => fromB64url(tok));
...
for (let outputBit = 0; outputBit < LONG_BITS; outputBit++) {
  ...
  const b_col = tokens.map(bytes => (bytes[byteIdx] >> bitIdx) & 1);

  const solution = solvGF2(A, b_col);
  if (!solution) {
	console.log(`WARNING: inconsistent system for output bit ${outputBit}`);
	newMasks.push(0n);
	allDetermined = false;
  } else {
	// Check if all 40 input bits are determined
	const undetermined = solution.filter(v => v === undefined).length;
	newMasks.push(maskBitsToNum(solution));
  }
}

// Verify
function computeSignWithMasks(mangaId, masks) {
  const pfx = Buffer.from([97,200,64,144,162,7,176,70,112,166,46,172,221,0,253,31,196,10,25,32,99,136,29,229,210,13,150,51,132,252,213,72,16,222,85,16,49,197,175,230,100,6,120,233,32,249,167,132,106]);
  const sfx = Buffer.from([127,241,120,206,4,167,103,234,234,27,134]);
  const bytes = Buffer.alloc(49+5+sfx.length);
  pfx.copy(bytes);
  const fb = buildFeatureBits(mangaId, 5);
  for (let ob = 0; ob < 40; ob++) {
	let p = 0;
	for (let b = 0; b < fb.length; b++) {
	  if (fb[b] && ((masks[ob] >> BigInt(b)) & 1n)) p ^= 1;
	}
...
let pass = 0, fail = 0;
for (const [id, expected] of oracles_long) {
  const got = computeSignWithMasks(id, newMasks);
...
console.log(`Verification: ${pass}/${oracles_long.length} pass, ${fail} fail`);

// Output C# arrays
console.log('\n--- C# LiveVariableBitMasks (long, 40 elements) ---');
const lines = [];
for (let i = 0; i < 40; i += 8) {
  lines.push('\t\t' + newMasks.slice(i, i+8).map(n=>n.toString()).join(', ') + ',');
}
